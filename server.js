...
function sbRequestOnce(method, endpoint, body, extraHeaders = {}) {
...
async function sbRequest(method, endpoint, body, extraHeaders = {}) {
  const retries = 3;
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      return await sbRequestOnce(method, endpoint, body, extraHeaders);
    } catch(e) {
      ...
      console.log(`⏳ Supabase retry ${attempt + 1}/${retries} in ${delay / 1000}s... (${e.message})`);
...
// Warm up Supabase connection on start
sb.select('gd_users', 'limit=1').then(() => {
  console.log('✅ Supabase connected');
}).catch(e => {
  console.warn('⚠️ Supabase warm-up failed (will retry on first request):', e.message);
});
